...
#include "motor.h" 
#include "Courier_Reg.h"
#include "Courier_bold.h"
...
//gpio_direction_output(57,1);// --------------logic power -----------

fs = get_fs();
set_fs(get_ds());
fp_paper_sensing = filp_open("/sys/class/gpio/gpio507/value", O_RDONLY, 0600);
vfs_read(fp_paper_sensing, paper, 1, &fp_paper_sensing->f_pos);
set_fs(fs);
filp_close(fp_paper_sensing, NULL);
...
if(paper[0]=='0')
{
	...
	fs = get_fs();
        set_fs(get_ds());  
	vfs_write(f2, "0", 1, &f2->f_pos);
	set_fs(fs);	
//	printk(KERN_ALERT "...file created...\n");	
	filp_close(f2,NULL);

//gpio_direction_output(57,0);// --------------logic power -----------
}

/*
	f_LB = filp_open("/opt/daemon_files/LB", O_RDONLY, 0);
	fs = get_fs();
	set_fs(get_ds());
	vfs_read(f_LB, LB_buf1, 2, &f->f_pos);
	set_fs(fs);
...
//gpio_direction_output(54,1);// --------------logic power -----------
...
//---------------------------------------------------------------------------------------------
			...
			else if(data[data_size-1]==44)  // left -------------------------
			{
			allignment=1;
			}
...
if(size==1)  // Small font - starts
{
				...
				for(height=0;height<16;height++)
				{
					for(width=1;width<=38 && width<(data_size-3);width++)
					{
						...
						switch(fontstyle){
	case 1:
		envy[width]=DroidSans_R_10_10_15[data[width]][height];
	break;	
	case 2:
		envy[width]=Courier10reg[data[width]][height];
	break;
	default:
		envy[width]=DroidSans_R_10_10_15[data[width]][height];
	break;					
...
switch(fontstyle){
	case 1:
		envy[width]=DroidSans_B_10_10_15[data[width]][height];
	break;	
	case 2:
		envy[width]=Courier10bold[data[width]][height];
	break;
	default:
		envy[width]=DroidSans_B_10_10_15[data[width]][height];
	break;
...
if(size==2)
{
				...
				for(height=0;height<23;height++)
				{
					for(width=1;width<=32 && width<(data_size-3);width++)
					{
						...
						if(font==1)
						{
//						envy[width]=DroidSansMono_R_14_12_22[data[width]][height];
						envy[width]=Lekton_R_16_12_22[data[width]][height];
						}
						else if(font==2)
						{
//						envy[width]=LuxiMono_R_14_12_22[data[width]][height];
						envy[width]=BitstreamVeraSansMono_R_14_12_22[data[width]][height];
						}
...
if(size==3)   // Large - starts
{
				...
				{
					for(width=1;width<=26 && width<(data_size-3);width++)
					{

						if(font==1)
						{
//						envy[width]=DroidSansMono_R_14_12_22[data[width]][height];
						envy[width]=Lekton_R_18_16_26[data[width]][height];
						}
						else if(font==2)
						{
//						envy[width]=LuxiMono_R_14_12_22[data[width]][height];
						envy[width]=BitstreamVeraSansMono_R_18_16_26[data[width]][height];
						}
...
gpio_direction_output(54,0);// --------------logic power -----------
}
else
{
	f2 = filp_open("/usr/share/status/PRINTER_status", O_WRONLY, 0644);
...
static int __init printer_init(void)   
{
	...
	gpio_request(68,"strobe5&6");
	gpio_export(68,true);

	gpio_direction_output(67,0); // ------------ SPI latch default---------
//	gpio_direction_output(57,1);// --------------logic power -----------
	gpio_direction_output(117,1);// --------------logic power ----------- //Rajesh
...
//14
			...
			tmp_buff[0]=((envy[1] & 0xff00) >> 8 );
			tmp_buff[1]=((envy[1] & 0x00fc) | ((envy[2] & 0xc000) >> 14));
			tmp_buff[2]=(((envy[2] & 0x3fc0) >> 6 ));
			tmp_buff[3]=((envy[2] & 0x003c) << 2 | ((envy[3] & 0xf000) >> 12));
			tmp_buff[4]=(((envy[3] & 0x0ff0) >> 4 ));
			tmp_buff[5]=((envy[3] & 0x000c) << 4 | ((envy[4] & 0xfc00) >> 10));
			tmp_buff[6]=(envy[4] & 0x00ff);

			tmp_buff[7]=((envy[5] & 0xff00) >> 8 );
			tmp_buff[8]=((envy[5] & 0x00fc) | ((envy[6] & 0xc000) >> 14));
			tmp_buff[9]=(((envy[6] & 0x3fc0) >> 6 ));
			tmp_buff[10]=((envy[6] & 0x003c) << 2 | ((envy[7] & 0xf000) >> 12));
			tmp_buff[11]=(((envy[7] & 0x0ff0) >> 4 ));
			tmp_buff[12]=((envy[7] & 0x000c) << 4 | ((envy[8] & 0xfc00) >> 10));
			tmp_buff[13]=(envy[8] & 0x00ff);

			tmp_buff[14]=((envy[9] & 0xff00) >> 8 );
			tmp_buff[15]=((envy[9] & 0x00fc) | ((envy[10] & 0xc000) >> 14));
			tmp_buff[16]=(((envy[10] & 0x3fc0) >> 6 ));
			tmp_buff[17]=((envy[10] & 0x003c) << 2 | ((envy[11] & 0xf000) >> 12));
			tmp_buff[18]=(((envy[11] & 0x0ff0) >> 4 ));
			tmp_buff[19]=((envy[11] & 0x000c) << 4 | ((envy[12] & 0xfc00) >> 10));
			tmp_buff[20]=(envy[12] & 0x00ff);

			tmp_buff[21]=((envy[13] & 0xff00) >> 8 );
			tmp_buff[22]=((envy[13] & 0x00fc) | ((envy[14] & 0xc000) >> 14));
			tmp_buff[23]=(((envy[14] & 0x3fc0) >> 6 ));
			tmp_buff[24]=((envy[14] & 0x003c) << 2 | ((envy[15] & 0xf000) >> 12));
			tmp_buff[25]=(((envy[15] & 0x0ff0) >> 4 ));
			tmp_buff[26]=((envy[15] & 0x000c) << 4 | ((envy[16] & 0xfc00) >> 10));
			tmp_buff[27]=(envy[16] & 0x00ff);

			tmp_buff[28]=((envy[17] & 0xff00) >> 8 );
			tmp_buff[29]=((envy[17] & 0x00fc) | ((envy[18] & 0xc000) >> 14));
			tmp_buff[30]=(((envy[18] & 0x3fc0) >> 6 ));
			tmp_buff[31]=((envy[18] & 0x003c) << 2 | ((envy[19] & 0xf000) >> 12));
			tmp_buff[32]=(((envy[19] & 0x0ff0) >> 4 ));
			tmp_buff[33]=((envy[19] & 0x000c) << 4 | ((envy[20] & 0xfc00) >> 10));
			tmp_buff[34]=(envy[20] & 0x00ff);

			tmp_buff[35]=((envy[21] & 0xff00) >> 8 );
			tmp_buff[36]=((envy[21] & 0x00fc) | ((envy[22] & 0xc000) >> 14));
			tmp_buff[37]=(((envy[22] & 0x3fc0) >> 6 ));
			tmp_buff[38]=((envy[22] & 0x003c) << 2 | ((envy[23] & 0xf000) >> 12));
			tmp_buff[39]=(((envy[23] & 0x0ff0) >> 4 ));
			tmp_buff[40]=((envy[23] & 0x000c) << 4 | ((envy[24] & 0xfc00) >> 10));
			tmp_buff[41]=(envy[24] & 0x00ff);

			tmp_buff[42]=((envy[25] & 0xff00) >> 8 );
			tmp_buff[43]=((envy[25] & 0x00fc) | ((envy[26] & 0xc000) >> 14));
			tmp_buff[44]=(((envy[26] & 0x3fc0) >> 6 ));
			tmp_buff[45]=((envy[26] & 0x003c) << 2 | ((envy[27] & 0xf000) >> 12));
			tmp_buff[46]=(((envy[27] & 0x0ff0) >> 4 ));
			tmp_buff[47]=((envy[27] & 0x000c) << 4 | ((envy[28] & 0xfc00) >> 10));
			tmp_buff[48]=(envy[28] & 0x00ff);
