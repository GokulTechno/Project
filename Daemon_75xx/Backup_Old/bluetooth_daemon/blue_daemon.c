...
static void blue_en(void)
{
	...
	struct termios oldtio,newtio;
        mux_fd[0].fd = open("/dev/mux2", O_RDWR | O_NOCTTY );
...
{	
   ...
   // write(fd,btstate,sizeof(btstate));
    ...
    if (retval2 && (mux_fd[0].revents & POLLWRNORM)) {
        write(mux_fd[0].fd,btstate,sizeof(btstate));
    }
    while(state_stat)
    {
...
if(bscan_buff[0]=='1')
{
   ...
   // write(fd,btscan,sizeof(btscan));
    ...
    int retval2 = poll(mux_fd, 1, 1000);
    if (retval2 && (mux_fd[0].revents & POLLWRNORM)) {
        write(mux_fd[0].fd, btscan,sizeof(btscan));
    }
...
{	
	...
	struct termios oldtio,newtio;
   	...
   	newtio.c_lflag = ICANON;
        ...
        int retval2 = poll(mux_fd, 1, 1000);
        if (retval2 && (mux_fd[0].revents & POLLWRNORM)) {
            write(mux_fd[0].fd,btoff,sizeof(btoff));
        }
	int state_read =1;
	while(state_read)
	{
