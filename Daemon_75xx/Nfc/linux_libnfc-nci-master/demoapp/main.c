...
static void skeleton_daemon()
{
    ...
    /* Open the log file */
    openlog ("nfc_daemon.log", LOG_PID, LOG_DAEMON);
}

FILE *file;
unsigned char pdata[2046];
struct pollfd printer_fd[1];
...
void PrintNDEFContent(nfc_tag_info_t* TagInfo, ndef_info_t* NDEFinfo, unsigned char* ndefRaw, unsigned int ndefRawLen)
{
    ...
    else
    {
        ...
        for(i = 0x00; i < ndefRawLen; i++)
        {
            ...
            if(i > 0x03 )
            {
                sprintf(&pdata[j], "%c", NDEFContent[i]);
                j++;
            }
            if(i%30 == 0 && 0x00 != i)
            {
        ...
        printf("Text Data:%s\n",pdata);
        printer_fd[0].fd = open("/dev/printer", O_WRONLY);
        printer_fd[0].events = POLLOUT | POLLWRNORM;
        int retval = poll(printer_fd, 1, 1000);
        if (retval && (printer_fd[0].revents & POLLWRNORM)) {
            write(printer_fd[0].fd, pdata, sizeof(pdata));
        }
        close(printer_fd[0].fd);
        file = fopen("/dev/printer","w");
        fprintf(file,"%s", pdata);
        fclose(file);
...
int WaitDeviceArrival(int mode, unsigned char* msgToSend, unsigned int len)
{
    ...
    do
    {
        ...
        if(eDevState_PRESENT == g_DevState)
        {
            ...
            fclose(nfclog);
            nfclog = fopen("/opt/daemon_files/nfc_data","a");
            ...
            if(eDevType_TAG == g_Dev_Type)
            {
                ...
                {
                    printf("		'Type Unknown'\n");
                    fprintf(nfclog,"%s","Type: Unknown\n");
                } break;
                case TARGET_TYPE_ISO14443_3A:
                {
                    printf("		'Type A'\n");
                    fprintf(nfclog,"%s","Type: Type A\n");
                } break;
                case TARGET_TYPE_ISO14443_3B:
                {
                    printf("		'Type 4B'\n");
                    fprintf(nfclog,"%s","Type: Type 4B\n");
                } break;
                case TARGET_TYPE_ISO14443_4:
                {
                    printf("		'Type 4A'\n");
                    fprintf(nfclog,"%s","Type: Type 4A\n");
                } break;
                case TARGET_TYPE_FELICA:
                {
                    printf("		'Type F'\n");
                    fprintf(nfclog,"%s","Type: Type F\n");
                } break;
                case TARGET_TYPE_ISO15693:
                {
                    printf("		'Type V'\n");
                    fprintf(nfclog,"%s","Type: Type V\n");
                } break;
                case TARGET_TYPE_NDEF:
                {
                    printf("		'Type NDEF'\n");
                    fprintf(nfclog,"%s","Type: Type NDEF\n");
                } break;
                case TARGET_TYPE_NDEF_FORMATABLE:
                {
                    printf("		'Type Formatable'\n");
                    fprintf(nfclog,"%s","Type: Type Formatable\n");
                } break;
                case TARGET_TYPE_MIFARE_CLASSIC:
                {
                    printf("		'Type A - Mifare Classic'\n");
                    fprintf(nfclog,"%s","Type: Type A - Mifare Classic\n");
                } break;
                case TARGET_TYPE_MIFARE_UL:
                {
                    printf("		'Type A - Mifare Ul'\n");
                    fprintf(nfclog,"%s","Type: Type A - Mifare Ul\n");
                } break;
                case TARGET_TYPE_KOVIO_BARCODE:
                {
                    printf("		'Type A - Kovio Barcode'\n");
                    fprintf(nfclog,"%s","Type: Type A - Kovio Barcode\n");
                } break;
                case TARGET_TYPE_ISO14443_3A_3B:
                {
                    printf("		'Type A/B'\n");
                    fprintf(nfclog,"%s","Type: Type A/B\n");
                } break;
                default:
                {
                    printf("		'Type %d (Unknown or not supported)'\n", TagInfo.technology);
                    fprintf(nfclog,"%s","Type: Type %d (Unknown or not supported)\n", TagInfo.technology);
                } break;
                }
                ...
                if((0x00 != TagInfo.uid_length) && (32 >= TagInfo.uid_length))
                {
                    ...
                    {
                        printf("		NFCID1 :    \t'");
                        fprintf(nfclog,"%s","NFCID1 : ");
                    }
                    else if(8 == TagInfo.uid_length)
                    {
                        printf("		NFCID2 :    \t'");
                        fprintf(nfclog,"%s","NFCID2 : ");
                    }
                    else
                    {
                        printf("		UID :       \t'");
                        fprintf(nfclog,"%s","UID : ");
                    }

                    for(i = 0x00; i < TagInfo.uid_length; i++)
                    {
                        printf("%02X ", (unsigned char) TagInfo.uid[i]);
                        fprintf(nfclog,"%02X ", (unsigned char) TagInfo.uid[i]);
                    }
                    printf("'\n");
                    fprintf(nfclog,"%s","\n");
                }
